...
export default function AddCategoryPanel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const res = await apiPost('/categories', formData);
      ...
      <form onSubmit={handleSubmit} className={styles.panelForm}>
        <div className="grid grid-cols-1 gap-y-5 sm:grid-cols-6">
          <div className="sm:col-span-4">
            <label className="block text-sm font-medium text-white mb-1">Category name</label>
        ...
        <div className={styles.formActions}>
          <button type="submit" className={styles.addBtn}>Add Category</button>
        </div>
      </form>
    </>
  );
