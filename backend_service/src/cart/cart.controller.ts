...
export class CartController {
    ...
    @Post()
    async addToCart(@Body() cart: { user_id: ParseUUIDPipe, product_id: ParseUUIDPipe, quantity: number }): Promise<ApiResponse> {
        try {
            const result = await this.cartService.addToCart(cart);
            return new ApiResponse(true, result.message, result.payload);
        } catch (err) {
    ...
    async getCartQuantity(@Param('cart_id') cart_id: ParseUUIDPipe): Promise<ApiResponse> {
        try {
    ...
    async deleteCart(@Param('cart_id') cart_id: ParseUUIDPipe): Promise<ApiResponse> {
        try {
    ...
    async deleteCartItem(@Param('cart_item_id') cart_item_id: ParseUUIDPipe): Promise<ApiResponse> {
        try {
    ...
    @Put('qty-update')
    async updateCartQty(@Body() body: { product_id: ParseUUIDPipe, cart_item_id: ParseUUIDPipe, opType: string, cart_id: ParseUUIDPipe }): Promise<ApiResponse> {
        try {
    ...
    @Get(':cart_id')
    async getCartItems(@Param('cart_id') cart_id: ParseUUIDPipe): Promise<ApiResponse> {
        const result = await this.cartService.getCartItems(cart_id);
        return new ApiResponse(true, 'Cart items retrieved successfully', result);
    }
    ...
    async getProductStock(@Param('product_id') product_id: ParseUUIDPipe): Promise<ApiResponse> {
        try {
    ...
    async getCartItemQty(@Param('product_id') product_id: ParseUUIDPipe, @Param('cart_id') cart_id: ParseUUIDPipe): Promise<ApiResponse> {
        try {
